...
(setq-default
 ...
 customize-save-defaults nil)                  ; Customize를 기본적으로 저장하지 않도록 설정

;; read-extended-command-predicate 설정
...
(use-package evil
  ...
  (advice-add 'evil-collection-mu3e-update-main-view :override #'evil-collection-mu4e-update-main-view@override)
...
(with-eval-after-load 'evil-collection
  (evil-collection-define-key 'normal 'elfeed-search-mode-map
    "f" #'my/elfeed-toggle-flagged
    "F" #'my/elfeed-show-flagged
    "E" #'my/elfeed-export-flagged))
#+end_src
...
(use-package go-mode
  ...
  (defun my/go-mode-setup()
    (setq tab-width 2
          compile-command (if (string-match "_test\\.go$" (buffer-file-name))
                              "go test -v ."
                            "go run -v ."))
...
(use-package go-mode
  ...
  (defun my/go-mode-setup()
    ...
    (when (buffer-file-name)
      (setq compile-command (if (string-match "_test\\.go$" (buffer-file-name))
                                "go test -v ."
                              "go run -v .")))
...
.org-string { color: %s !important; }
  ...
  (advice-add 'disable-theme :after
              (lambda (&rest _) (my/update-org-html-theme)))
...
(use-package mu4e
  :if (or (not (eq system-type 'windows-nt))
          (executable-find "mu"))  ; Windows에서는 수동 설치된 mu가 있을 때만
  :straight ( :type git
              :host github
              :repo "djcb/mu"
              :branch "v1.12.12"
...
(keymap-set my/key-map "<f5>"
            (lambda () (interactive) (find-file "~/org/inbox.org")))
(keymap-set my/key-map "<f6>" 'mu4e)
(keymap-set my/key-map "<f7>" 'elfeed-summary)
#+end_src
